...
_FEATURES_DIM = {
    constants.DEIT_TSCAM_BASE_P16_224: 768,
    constants.DEIT_TSCAM_SMALL_P16_224: 384,
    constants.DEIT_TSCAM_TINY_P16_224: 192
}
...
class TSCAM(VisionTransformer):
    def __init__(self,
        ...
        self.x_logits = None # Logits

        self.encoder_last_features = None
        self.encoder_n_out_channels = _FEATURES_DIM[encoder_name]
    ...
    def forward(self, x, labels=None):
        ...
        self.encoder_last_features = x_patch  # bsz, ?, h, w

        x_patch = self.dropout_2d(x_patch)

        # SFUDA -----
        ft = self.avgpool(x_patch)
        
        ft = ft.reshape(ft.size(0), -1)
        self.lin_ft = ft  # bsz, sz
        #self.lin_ft = x_patch.clone()  # bsz, sz
        #
        # self.lin_ft_x_patch = x_patch.clone()
        # -----------


        d = ft.shape[0]

        x_patch =  ft.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 14, 14)
    ...
    def get_linear_weights(self):
        return nn.AdaptiveAvgPool2d(1)(self.head.weight).squeeze(3).squeeze(2)
...
def run_TSCAM():
    ...
    for encoder in [constants.DEIT_TSCAM_BASE_P16_224,
        ...
        bsize = 1
        h, w = 224, 224
        x = torch.rand(bsize, 3, h, w).to(device)
