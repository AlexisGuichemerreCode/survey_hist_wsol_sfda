...
def get_loss(output_dict, gt_labels,
             ce_label_smoothing: float = 0.0,
             **kwargs):
    return nn.CrossEntropyLoss(label_smoothing=ce_label_smoothing)(
        output_dict['logits'], gt_labels.long()) + \
           nn.CrossEntropyLoss(label_smoothing=ce_label_smoothing)(
               output_dict['logit_b'], gt_labels.long())
